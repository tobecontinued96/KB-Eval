...
```mermaid
flowchart LR
    User["研发 / 测试 / 交付验证"] --> Web["React + Vite 前端<br/>:5598"]
    Web --> API["FastAPI 后端<br/>:8200"]
    API --> DB[("PostgreSQL 16<br/>runs / run_summaries / run_reports")]
    API --> Dataset["评测集服务<br/>生成 / 编辑 / 审核"]
    Dataset --> Parser["MarkItDown"]
    Dataset --> Files["datasets/*.jsonl<br/>datasets/generated/*.jsonl"]
    API --> Runner["评测 Runner<br/>kb_eval.runner"]
    Runner --> Dify["Dify 知识库 API"]
